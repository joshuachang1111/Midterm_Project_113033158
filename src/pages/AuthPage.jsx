...
export default function AuthPage() {
  ...
  function getThicknessShadow(rotateX, rotateY) {
    // 根據傾斜角度動態計算厚度陰影，模擬長方體側面
    const depth = 16;
    const shadowX = (rotateY / 12) * depth;
    const shadowY = (-rotateX / 12) * depth;
    return [
      // 厚度層（模擬側面）
      `${shadowX * 0.25}px ${shadowY * 0.25}px 0px #D4C4A8`,
      `${shadowX * 0.5}px ${shadowY * 0.5}px 0px #C8B89A`,
      `${shadowX * 0.75}px ${shadowY * 0.75}px 0px #BCA98C`,
      `${shadowX}px ${shadowY}px 0px #B09A7E`,
      // 外部陰影
      `${shadowX + 8}px ${shadowY + 20}px 40px rgba(0,0,0,0.2)`,
    ].join(", ");
  }
  ...
  function handleMouseMove(e) {
    ...
    card.style.boxShadow = getThicknessShadow(rotateX, rotateY);
  }
  ...
  function handleMouseLeave() {
    ...
    card.style.boxShadow = "0px 8px 0px #C8B89A, 0px 20px 40px rgba(0,0,0,0.15)";
  }
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-[#E8D5B7] via-[#F5ECD7] to-[#FAF7F2] p-4">
      <div
        ...
        onAnimationEnd={() => {
          ...
          // 動畫結束後給一個預設的厚度陰影
          if (cardRef.current) {
            cardRef.current.style.boxShadow = "0px 8px 0px #C8B89A, 0px 20px 40px rgba(0,0,0,0.15)";
          }
        }}
        className={`w-full max-w-md bg-white/70 backdrop-blur-md rounded-3xl p-8 border border-[#E8D5B7] ${animated ? "animate-flip-in" : ""}`}
      ...
      >
        <div style={{ transform: "translateZ(30px)" }}>
          ...
          <div className="text-center mb-8">
            <div className="text-5xl mb-2" style={{ transform: "translateZ(10px)", display: "block" }}>💬</div>
